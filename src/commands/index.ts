...
export function toggleFileAlias() {
    return commands.registerCommand(
        `${util.CMND_NAME}.toggleFileAlias`,
        async(e) => {
            ...
            if (groupIndex > -1) {
                list[groupIndex].documents = list[groupIndex].documents.map(
                    (item) => {
                        if (
                            item == doc
            ...
            util.showMsg(
                `file alias ${type} for "${util.getFileName(util.getDocPath(doc))}"`,
                false,
            )
        },
    )
...
export async function addCurrentFile(e) {
    ...
    if (groupName) {
        ...
        try {
            ...
            util.showMsg(
                `file "${util.getDocLabel(filePath)}" added to "${groupName}"`,
                false,
            )
        } catch (error) {
            // console.error(error);
...
export function deleteFile() {
    return commands.registerCommand(
        ...
        async(e) => {
            ...
            if (isDefaultGroup) {
                ...
                if (typeof filePath == 'object') {
                    list[groupIndex].documents.splice(
                        list[groupIndex].documents.findIndex(
                            (item) => item.alias == filePath.alias,
                        ),
                        1,
                    )
                } else {
            ...
            util.showMsg(
                `file "${util.getDocLabel(filePath, true)}" removed from "${groupName}"`,
                false,
            )
        },
    )
    ...
    return commands.registerCommand(
        ...
        async(e) => {
            ...
            // cmnd panel
                ...
                const gi = group
                    ? list.findIndex((item) => item?.name === group)
                    : -1
                        ...
                        .then(async(yes) => {
                            if (yes) {
                                if (group === util.defGroup) {
                                    await util.updateConfForScope(
                                        'list',
                                        util.getListByType(list, 'object'),
                                        scope,
                                    )

                                    util.showMsg(`"${group}" files are removed`)
                                } else {
                                    await removeGroupAndChild(
                                        list,
                                        group,
                                        scope,
                                    )
...
export function changeFileGroup() {
    return commands.registerCommand(
        `${util.CMND_NAME}.changeFileGroup`,
        async(e) => {
            const scope: util.Scope | undefined = e?.scope
            ...
            util.showMsg(
                `"${util.getDocLabel(doc)}" moved from "${group}" to "${toGroup}"`,
                false,
            )
        },
    )
...
export function treeFileNameDisplay() {
    return commands.registerCommand(
        `${util.CMND_NAME}.DisplayFileNameInListAs`,
        () => {
            const displayType
                = util.getConf('DisplayFileNameInListAs')
...
function addToList(list: any[], file: any) {
    const exists = list.some(function(item) {
        return typeof item === 'object' ? item.filePath === file : item === file
    })
